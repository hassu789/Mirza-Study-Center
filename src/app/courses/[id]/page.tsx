      ...
      <div className="flex min-h-screen flex-col bg-white dark:bg-zinc-950">
        ...
        <div className="flex flex-1 items-center justify-center">
          <div className="text-center">
            <div className="mx-auto mb-6 flex h-24 w-24 items-center justify-center rounded-full bg-slate-100 dark:bg-zinc-800">
              <svg className="h-12 w-12 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
            </div>
            <h1 className="mb-2 text-2xl font-bold text-slate-900 dark:text-white">Course Not Found</h1>
            <p className="mb-6 text-slate-600 dark:text-slate-400">The course you&apos;re looking for doesn&apos;t exist.</p>
            <Link
              href="/courses"
              className="inline-flex items-center gap-2 rounded-xl bg-violet-600 px-6 py-3 font-semibold text-white transition-all hover:bg-violet-700"
            >
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-16 lg:px-8">
          <AnimatedSection animation="fade-up">
            ...
            <div className="mb-3 flex flex-wrap gap-2 sm:mb-4 sm:gap-3">
              <span className="rounded-full bg-violet-600 px-3 py-1 text-xs font-semibold text-white shadow-lg sm:px-4 sm:py-1.5 sm:text-sm">
                {course.category}
              </span>
          ...
          <div className="grid gap-6 lg:grid-cols-3 lg:gap-8">
            ...
            <div className="lg:col-span-2 space-y-6 sm:space-y-8">
              ...
              <AnimatedSection animation="fade-up">
                <div className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900 sm:p-6">
                  <h2 className="mb-4 flex items-center gap-2 text-lg font-bold text-slate-900 dark:text-white sm:mb-6 sm:gap-3 sm:text-xl">
                    <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-violet-100 text-sm text-violet-600 dark:bg-violet-900/30 dark:text-violet-400 sm:h-10 sm:w-10 sm:text-base">
                      📚
                    </span>
                    What You&apos;ll Learn
                  </h2>
                  <div className="grid gap-2 sm:grid-cols-2 sm:gap-3">
                    {whatYouWillLearn.map((item, index) => (
                      <div key={index} className="flex items-start gap-2 sm:gap-3">
                        ...
                        <span className="text-xs text-slate-600 dark:text-slate-400 sm:text-sm">{item}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900 sm:p-6">
                  ...
                  <div className="space-y-2 sm:space-y-3">
                    {curriculum.map((item, index) => (
                      <div
                        key={index}
                        className="group rounded-xl border border-slate-100 bg-slate-50 p-3 transition-all hover:border-violet-200 hover:bg-violet-50/50 dark:border-zinc-700 dark:bg-zinc-800 dark:hover:border-violet-800 dark:hover:bg-violet-900/20 sm:p-4"
                      >
                        <div className="flex items-center justify-between gap-2">
                          <div className="flex items-center gap-2 sm:gap-4">
                            <span className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-xl bg-white text-sm font-bold text-violet-600 shadow-sm dark:bg-zinc-700 dark:text-violet-400 sm:h-10 sm:w-10 sm:text-base">
                              {index + 1}
                            </span>
                            <div className="min-w-0">
                              <span className="text-[10px] font-semibold uppercase text-violet-600 dark:text-violet-400 sm:text-xs">{item.module}</span>
                              <h3 className="truncate text-sm font-semibold text-slate-900 dark:text-white sm:text-base">{item.title}</h3>
                            </div>
                          </div>
                ...
                <AnimatedSection animation="fade-up" delay={200}>
                  <div className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900 sm:p-6">
                    ...
                    <div className="flex flex-col gap-4 sm:flex-row sm:gap-6">
                      <div className="relative mx-auto h-24 w-24 flex-shrink-0 overflow-hidden rounded-2xl border-4 border-violet-200 shadow-lg dark:border-violet-800 sm:mx-0 sm:h-32 sm:w-32">
                        <img
                      ...
                      <div className="text-center sm:text-left">
                        <h3 className="text-base font-bold text-slate-900 dark:text-white sm:text-lg">{instructor.name}</h3>
                        <p className="mb-1 text-sm text-violet-600 dark:text-violet-400 sm:mb-2">{instructor.subject} Expert</p>
                        ...
                        <p className="text-xs text-slate-600 dark:text-slate-400 sm:text-sm">{instructor.bio}</p>
                      </div>
                    </div>
                  </div>
                </AnimatedSection>
            ...
            <div className="lg:col-span-1">
              <AnimatedSection animation="fade-left">
                <div className="sticky top-24 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl dark:border-zinc-800 dark:bg-zinc-900">
                  {/* Course Image */}
                  ...
                  <div className="p-4 sm:p-6">
                    <div className="mb-4 space-y-3 sm:mb-6">
                      <div>
                        <span className="text-3xl font-bold text-slate-900 dark:text-white sm:text-4xl">₹{course.price.toLocaleString('en-IN')}</span>
                        <span className="text-sm text-slate-500 dark:text-slate-400">/month per subject</span>
                      </div>
                      {course.price6Months && (
                    ...
                    <Link
                      href="/inquiry"
                      className="flex w-full items-center justify-center gap-2 rounded-xl border-2 border-slate-200 py-3 text-sm font-semibold text-slate-700 transition-all hover:border-violet-200 hover:bg-violet-50 dark:border-zinc-700 dark:text-slate-300 dark:hover:border-violet-800 dark:hover:bg-violet-900/20 sm:py-4 sm:text-base"
                    >
                      Request Demo Class
                    </Link>

                    <div className="mt-4 space-y-3 border-t border-slate-100 pt-4 dark:border-zinc-800 sm:mt-6 sm:space-y-4 sm:pt-6">
                      <div className="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-400 sm:gap-3 sm:text-sm">
